...
Two_Platform_by_CIA <-
        ConfLeis_Explo %>% 
        summarise(
                  Talk_t_colleagues = n_distinct(Talk_t_colleagues),
                  Read_book = n_distinct(Read_book),
                  Play_instrument = n_distinct(Play_instrument)
                  )
...
ConfLeisWC1 <- c(WestCarina[1:dimWC[2], 5:6],  WestCarina[1:dimWC[2], 22:28])
ConfLeisWC2 <- WestCarina[1:dimWC[2], 22:28]
ConfLeisWC <- merge(ConfLeisWC1, ConfLeisWC2,all.x = TRUE)

# Dimentions of new data
dim(ConfLeisWC)
